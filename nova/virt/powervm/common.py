...
def ssh_connect(connection):
    ...
    """
    LOG.info('AAAAAAAA host=%s, user=%s, passwd=%s', connection.host, connection.username, connection.password)
    try:
...
def ftp_get_command(connection, remote_path, local_path):
    ...
    """
    try:
        LOG.info('FTPPPPPPPPPP host=%s, user=%s, passwd=%s',connection.host, connection.username,connection.password)
        ftp = ftplib.FTP(host=connection.host,
                         user=connection.username,
                         passwd=connection.password)
        ...
        LOG.info('NNNNNNNNNNNNNN name=%s, local_path=%s', name, local_path)
#        with open(local_path, 'wb') as ftpfile:
        ftpfile = open(local_path, 'wb')
        LOG.info('NNNNNNNNNNNNN 1111')
        ftpcmd = 'RETR %s' % name
        LOG.info('NNNNNNNNNNNNNN 2222')
